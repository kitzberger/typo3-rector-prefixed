...
final class CheckoutEntityFactory
{
    public function run()
    {
        return \Typo3RectorPrefix20210415\SomeStaticClass::go();
    }
}
\class_alias('Typo3RectorPrefix20210415\\CheckoutEntityFactory', 'CheckoutEntityFactory', \false);
CODE_SAMPLE
, <<<'CODE_SAMPLE'
<?php
...
final class CheckoutEntityFactory
{
    ...
    public function setSomeStaticClass(\Typo3RectorPrefix20210415\SomeStaticClass $someStaticClass)
    {
...
, [self::STATIC_TYPES => ['SomeStaticClass']])]);
    ...
    private function processClass(\PhpParser\Node\Stmt\Class_ $class) : \PhpParser\Node\Stmt\Class_
    {
        foreach ($this->staticTypes as $implements => $staticType) {
            ...
            $variableName = $this->propertyNaming->fqnToVariableName($objectType);
            $paramBuilder = new \Typo3RectorPrefix20210415\Symplify\Astral\ValueObject\NodeBuilder\ParamBuilder($variableName);
    ...
    private function createSetEntityFactoryClassMethod(string $variableName, \PhpParser\Node\Param $param, \PhpParser\Node\Expr\Assign $assign) : \PhpParser\Node\Stmt\ClassMethod
    {
        ...
        $setEntityFactoryMethodBuilder = new \Typo3RectorPrefix20210415\Symplify\Astral\ValueObject\NodeBuilder\MethodBuilder($setMethodName);
