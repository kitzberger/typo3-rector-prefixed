...
use Typo3RectorPrefix20210223\Symfony\Component\Mime\Address;
...
final class SendNotifyEmailToMailApiRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Refactor ContentObjectRenderer::sendNotifyEmail to MailMessage-API', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
...
PHP
    ...
    {
        return new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign(new \PhpParser\Node\Expr\Variable(self::MAIL), $this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Core\Mail\MailMessage::class)])));
    }
    ...
    {
        return $this->nodeFactory->createMethodCall(self::MAIL, 'from', [new \PhpParser\Node\Expr\New_(new \PhpParser\Node\Name\FullyQualified(\Typo3RectorPrefix20210223\Symfony\Component\Mime\Address::class), [$this->nodeFactory->createArg(new \PhpParser\Node\Expr\Variable(self::SENDER_ADDRESS)), $this->nodeFactory->createArg(new \PhpParser\Node\Expr\Variable('senderName'))])]);
    }
    ...
    {
        return new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign(new \PhpParser\Node\Expr\Variable(self::PARSED_RECIPIENTS), $this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\MailUtility::class, 'parseAddresses', [$node->args[1]])));
    }
    ...
    {
        return new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign(new \PhpParser\Node\Expr\Variable(self::PARSED_REPLY_TO), $this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\MailUtility::class, 'parseAddresses', [$replyTo])));
    }
