...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210410\\Doctrine\\ORM\\EntityRepository'))) {
            return null;
        }
    ...
    private function resolveRepositoryName(\PhpParser\Node\Expr $expr) : string
    {
        ...
        $lastNamePart = (string) \Typo3RectorPrefix20210410\Nette\Utils\Strings::after($entityReferenceName, '\\', -1);
    ...
    private function guessRepositoryType(\PhpParser\Node\Expr $expr) : string
    {
        if ($expr instanceof \PhpParser\Node\Expr\ClassConstFetch) {
            ...
            $entityClassNamespace = (string) \Typo3RectorPrefix20210410\Nette\Utils\Strings::before($entityClass, '\\', -2);
            $lastNamePart = (string) \Typo3RectorPrefix20210410\Nette\Utils\Strings::after($entityClass, '\\', -1);
    ...
    private function refactorGetRepositoryMethodCall(\PhpParser\Node\Expr\MethodCall $methodCall) : ?\PhpParser\Node\Expr\MethodCall
    {
        ...
        if (\count($parentMethodCall->args) === 1) {
            ...
            if ($this->isObjectType($class, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210410\\Doctrine\\ORM\\EntityRepository'))) {
                return null;
            }
