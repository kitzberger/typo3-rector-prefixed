...
final class ParentGetterStmtsToExternalStmtsFactory
{
    ...
    public function __construct(\Rector\NodeTypeResolver\NodeTypeResolver $nodeTypeResolver, \Typo3RectorPrefix20210414\Symplify\Astral\NodeTraverser\SimpleCallableNodeTraverser $simpleCallableNodeTraverser, \Rector\Core\PhpParser\Comparing\NodeComparator $nodeComparator)
    {
    ...
    public function create(array $getUserStmts) : array
    {
        ...
        foreach ($getUserStmts as $key => $getUserStmt) {
            ...
            if (!$this->nodeTypeResolver->isObjectType($getUserStmt->expr, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210414\\Nette\\Security\\User'))) {
                continue;
            }
