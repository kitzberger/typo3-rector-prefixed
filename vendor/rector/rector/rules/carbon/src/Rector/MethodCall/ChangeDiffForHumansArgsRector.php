...
final class ChangeDiffForHumansArgsRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Typo3RectorPrefix20210401\\Change methods arguments of diffForHumans() on Carbon\\Carbon', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'CODE_SAMPLE'
use Carbon\Carbon;
...
CODE_SAMPLE
    ...
    {
        if (!$this->isOnClassMethodCall($node, 'Typo3RectorPrefix20210401\\Carbon\\Carbon', 'diffForHumans')) {
            return null;
        }
        ...
        if ($this->valueResolver->isTrue($secondArgValue)) {
            $node->args[1]->value = $this->nodeFactory->createClassConstFetch('Typo3RectorPrefix20210401\\Carbon\\CarbonInterface', 'DIFF_ABSOLUTE');
            return $node;
        }
        if ($this->valueResolver->isFalse($secondArgValue)) {
            $node->args[1]->value = $this->nodeFactory->createClassConstFetch('Typo3RectorPrefix20210401\\Carbon\\CarbonInterface', 'DIFF_RELATIVE_AUTO');
            return $node;
        }
        return null;
    }
