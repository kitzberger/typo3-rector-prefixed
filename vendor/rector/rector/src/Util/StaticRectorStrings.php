...
final class StaticRectorStrings
{
    ...
    public static function camelCaseToUnderscore(string $input) : string
    {
        ...
        $matches = \Typo3RectorPrefix20210422\Nette\Utils\Strings::matchAll($input, self::CAMEL_CASE_SPLIT_REGEX);
        $parts = [];
    ...
    public static function removePrefixes(string $value, array $prefixesToRemove) : string
    {
        foreach ($prefixesToRemove as $prefixToRemove) {
            if (\Typo3RectorPrefix20210422\Nette\Utils\Strings::startsWith($value, $prefixToRemove)) {
                $value = \Typo3RectorPrefix20210422\Nette\Utils\Strings::substring($value, \Typo3RectorPrefix20210422\Nette\Utils\Strings::length($prefixToRemove));
            }
        }
        return $value;
    }
    ...
    public static function removeSuffixes(string $value, array $suffixesToRemove) : string
    {
        foreach ($suffixesToRemove as $suffixToRemove) {
            if (\Typo3RectorPrefix20210422\Nette\Utils\Strings::endsWith($value, $suffixToRemove)) {
                $value = \Typo3RectorPrefix20210422\Nette\Utils\Strings::substring($value, 0, -\Typo3RectorPrefix20210422\Nette\Utils\Strings::length($suffixToRemove));
            }
        }
        return $value;
    }
