...
final class ControlDimFetchAnalyzer
{
    ...
    {
        return $this->matchNameOnVariableType($node, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210410\\Nette\\Application\\UI\\Form'));
    }
    public function matchNameOnControlVariable(\PhpParser\Node $node) : ?string
    {
        return $this->matchNameOnVariableType($node, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210410\\Nette\\Application\\UI\\Control'));
    }
    public function matchName(\PhpParser\Node $node) : ?string
    {
        ...
        if (!$this->isVariableTypes($node->var, [new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210410\\Nette\\ComponentModel\\IContainer')])) {
            return null;
        }
