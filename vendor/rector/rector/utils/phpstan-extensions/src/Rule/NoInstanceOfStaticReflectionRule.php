...
use Typo3RectorPrefix20210414\Symplify\PHPStanRules\Rules\AbstractSymplifyRule;
 ...
 */
final class NoInstanceOfStaticReflectionRule extends \Typo3RectorPrefix20210414\Symplify\PHPStanRules\Rules\AbstractSymplifyRule implements \PHPStan\Rules\Rule
{
    /**
     * @var string
     */
    ...
    public function __construct(\Typo3RectorPrefix20210414\Symplify\Astral\Naming\SimpleNameResolver $simpleNameResolver, \Rector\PHPStanExtensions\TypeAnalyzer\AllowedAutoloadedTypeAnalyzer $allowedAutoloadedTypeAnalyzer)
    {
    ...
    {
        return [\PhpParser\Node\Expr\Instanceof_::class, \PhpParser\Node\Expr\FuncCall::class];
    }
    /**
...
CODE_SAMPLE
     ...
     */
    private function resolveExprStaticType(\PhpParser\Node $node, \PHPStan\Analyser\Scope $scope) : ?\PHPStan\Type\Type
    {
        if ($node instanceof \PhpParser\Node\Expr\Instanceof_) {
            if ($node->class instanceof \PhpParser\Node\Name) {
                return new \PHPStan\Type\Constant\ConstantStringType($node->class->toString());
            }
