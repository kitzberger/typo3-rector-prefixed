...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210420\\Nette\\Http\\Request'))) {
            return null;
        }
        if (!$this->isName($node->name, 'getHeader')) {
            return null;
        }
        $requestName = $this->classMethodManipulator->addMethodParameterIfMissing($node, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210420\\Symfony\\Component\\HttpFoundation\\Request'), ['request', 'symfonyRequest']);
