...
final class PropertyNaming
{
    ...
    {
        $matches = \Typo3RectorPrefix20210223\Nette\Utils\Strings::match($methodName, self::GET_PREFIX_REGEX);
        if ($matches === null) {
            return null;
        }
    ...
    public function getExpectedNameFromType(\PHPStan\Type\Type $type) : ?\Rector\Naming\ValueObject\ExpectedName
    {
        ...
        foreach (self::EXCLUDED_CLASSES as $excludedClass) {
            if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::match($className, $excludedClass)) {
                return null;
            }
        }
        ...
        // remove "_"
        $shortClassName = \Typo3RectorPrefix20210223\Nette\Utils\Strings::replace($shortClassName, '#_#', '');
    ...
    {
        if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::contains($className, '\\')) {
            return \Typo3RectorPrefix20210223\Nette\Utils\Strings::after($className, '\\', -1);
        }
        return $className;
    }
    ...
    {
        // is SomeInterface
        if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::endsWith($shortClassName, self::INTERFACE)) {
            $shortClassName = \Typo3RectorPrefix20210223\Nette\Utils\Strings::substring($shortClassName, 0, -\strlen(self::INTERFACE));
        }
        // is ISomeClass
        if ($this->isPrefixedInterface($shortClassName)) {
            $shortClassName = \Typo3RectorPrefix20210223\Nette\Utils\Strings::substring($shortClassName, 1);
        }
        // is AbstractClass
        if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::startsWith($shortClassName, 'Abstract')) {
            $shortClassName = \Typo3RectorPrefix20210223\Nette\Utils\Strings::substring($shortClassName, \strlen('Abstract'));
        }
    ...
    private function prolongIfTooShort(string $shortClassName, string $className) : string
    {
        if (\in_array($shortClassName, ['Factory', 'Repository'], \true)) {
            $namespaceAbove = (string) \Typo3RectorPrefix20210223\Nette\Utils\Strings::after($className, '\\', -2);
            $namespaceAbove = (string) \Typo3RectorPrefix20210223\Nette\Utils\Strings::before($namespaceAbove, '\\');
    ...
    {
        if (!\Typo3RectorPrefix20210223\Nette\Utils\Strings::contains($fqn, '\\')) {
            return $fqn;
        }
        /** @var string $lastNamePart */
        $lastNamePart = \Typo3RectorPrefix20210223\Nette\Utils\Strings::after($fqn, '\\', -1);
        if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::endsWith($lastNamePart, self::INTERFACE)) {
            return \Typo3RectorPrefix20210223\Nette\Utils\Strings::substring($lastNamePart, 0, -\strlen(self::INTERFACE));
        }
        return $lastNamePart;
    }
    private function removeInterfaceSuffixPrefix(string $className, string $shortName) : string
    {
        ...
        // starts with "I\W+"?
        if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::match($shortName, self::I_PREFIX_REGEX)) {
            return \Typo3RectorPrefix20210223\Nette\Utils\Strings::substring($shortName, 1);
        }
        if (\Typo3RectorPrefix20210223\Nette\Utils\Strings::endsWith($shortName, self::INTERFACE)) {
            return \Typo3RectorPrefix20210223\Nette\Utils\Strings::substring($shortName, -\strlen(self::INTERFACE));
        }
        return $shortName;
    }
    /**
    ...
    private function getPrefixedClassMethods(\PhpParser\Node\Stmt\Property $property) : array
    {
        ...
        return \array_filter($classMethods, function (\PhpParser\Node\Stmt\ClassMethod $classMethod) : bool {
            ...
            return \Typo3RectorPrefix20210223\Nette\Utils\Strings::match($classMethodName, self::PREFIXED_CLASS_METHODS_REGEX) !== null;
        });
    }
    /**
    ...
    private function isPrefixedInterface(string $shortClassName) : bool
    {
        ...
        if (!\Typo3RectorPrefix20210223\Nette\Utils\Strings::startsWith($shortClassName, 'I')) {
            return \false;
        }
