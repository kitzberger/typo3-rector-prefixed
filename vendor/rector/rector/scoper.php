...
use Typo3RectorPrefix20210316\Nette\Utils\DateTime;
...
$dateTime = \Typo3RectorPrefix20210316\Nette\Utils\DateTime::from('now');
...
return [\Rector\Compiler\ValueObject\ScoperOption::PREFIX => 'RectorPrefix' . $timestamp, \Rector\Compiler\ValueObject\ScoperOption::FILES_WHITELIST => $filePathsToSkip, \Rector\Compiler\ValueObject\ScoperOption::WHITELIST => \Rector\Compiler\PhpScoper\StaticEasyPrefixer::getExcludedNamespacesAndClasses(), \Rector\Compiler\ValueObject\ScoperOption::PATCHERS => [
    ...
    function (string $filePath, string $prefix, string $content) : string {
        // skip vendor
        if (\Typo3RectorPrefix20210316\Nette\Utils\Strings::contains($filePath, 'vendor/')) {
            return $content;
        }
        // skip bin/rector.php for composer autoload class
        if (\Typo3RectorPrefix20210316\Nette\Utils\Strings::endsWith($filePath, 'bin/rector.php')) {
            return $content;
        }
        // skip scoper-autoload
        if (\Typo3RectorPrefix20210316\Nette\Utils\Strings::endsWith($filePath, 'vendor/scoper-autoload.php')) {
            return $content;
        }
    ...
    function (string $filePath, string $prefix, string $content) : string {
        // scoper missed PSR-4 autodiscovery in Symfony
        if (!\Typo3RectorPrefix20210316\Nette\Utils\Strings::endsWith($filePath, 'config.php') && !\Typo3RectorPrefix20210316\Nette\Utils\Strings::endsWith($filePath, 'services.php')) {
            return $content;
        }
        // skip "Rector\\" namespace
        if (\Typo3RectorPrefix20210316\Nette\Utils\Strings::contains($content, '$services->load(\'Rector')) {
            return $content;
        }
        return \Typo3RectorPrefix20210316\Nette\Utils\Strings::replace($content, '#services\\->load\\(\'#', 'services->load(\'' . $prefix . '\\');
    },
]];
