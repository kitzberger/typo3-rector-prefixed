...
final class PhpDocClassRenamer
{
    ...
    {
        $assertChoiceTagValueNode = $phpDocInfo->getByAnnotationClass('Typo3RectorPrefix20210409\\Symfony\\Component\\Validator\\Constraints\\Choice');
    ...
    {
        $doctrineAnnotationTagValueNode = $phpDocInfo->getByAnnotationClass('Typo3RectorPrefix20210409\\JMS\\Serializer\\Annotation\\Type');
        if (!$doctrineAnnotationTagValueNode instanceof \Rector\BetterPhpDocParser\PhpDoc\DoctrineAnnotationTagValueNode) {
            return;
        }
        foreach ($oldToNewClasses as $oldClass => $newClass) {
            ...
            if ($className) {
                ...
                $newContent = \Typo3RectorPrefix20210409\Nette\Utils\Strings::replace($className, '#\\b' . \preg_quote($oldClass, '#') . '\\b#', $newClass);
