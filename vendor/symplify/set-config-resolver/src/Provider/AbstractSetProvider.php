...
namespace Typo3RectorPrefix20210412\Symplify\SetConfigResolver\Provider;
...
abstract class AbstractSetProvider implements \Typo3RectorPrefix20210412\Symplify\SetConfigResolver\Contract\SetProviderInterface
{
    /**
     * @return string[]
     */
    ...
    public function provideByName(string $desiredSetName) : ?\Typo3RectorPrefix20210412\Symplify\SetConfigResolver\ValueObject\Set
    {
        ...
        } catch (\Typo3RectorPrefix20210412\Symplify\SymplifyKernel\Exception\ShouldNotHappenException $shouldNotHappenException) {
        }
        $message = \sprintf('Set "%s" was not found', $desiredSetName);
        throw new \Typo3RectorPrefix20210412\Symplify\SetConfigResolver\Exception\SetNotFoundException($message, $desiredSetName, $this->provideSetNames());
    }
    private function resolveSetUniquePathId(string $setPath) : string
    {
        $setPath = \Typo3RectorPrefix20210412\Nette\Utils\Strings::after($setPath, \DIRECTORY_SEPARATOR, -2);
        if ($setPath === null) {
