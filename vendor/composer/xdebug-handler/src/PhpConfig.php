...
class PhpConfig
{
    ...
    {
        if ($data = $this->getDataAndReset()) {
            \Typo3RectorPrefix20210418\Composer\XdebugHandler\Process::setEnv('PHPRC', $data['tmpIni']);
            \Typo3RectorPrefix20210418\Composer\XdebugHandler\Process::setEnv('PHP_INI_SCAN_DIR', '');
        }
        return array();
    }
    /**
    ...
    {
        if ($data = \Typo3RectorPrefix20210418\Composer\XdebugHandler\XdebugHandler::getRestartSettings()) {
            \Typo3RectorPrefix20210418\Composer\XdebugHandler\Process::setEnv('PHPRC', $data['phprc']);
            \Typo3RectorPrefix20210418\Composer\XdebugHandler\Process::setEnv('PHP_INI_SCAN_DIR', $data['scanDir']);
        }
        return $data;
    }
}
