...
use Typo3RectorPrefix20210405\Symplify\SetConfigResolver\ConfigResolver;
...
final class Typo3RectorConfigsResolver
{
    ...
    {
        $this->configResolver = new \Typo3RectorPrefix20210405\Symplify\SetConfigResolver\ConfigResolver();
        $rectorSetProvider = new \Ssch\TYPO3Rector\Set\Typo3RectorSetProvider(new \Rector\Set\RectorSetProvider());
        $this->setAwareConfigResolver = new \Typo3RectorPrefix20210405\Symplify\SetConfigResolver\SetAwareConfigResolver($rectorSetProvider);
    }
    /**
     * @return SmartFileInfo[]
     */
    public function resolveFromConfigFileInfo(\Typo3RectorPrefix20210405\Symplify\SmartFileSystem\SmartFileInfo $configFileInfo) : array
    {
    ...
    public function provide() : \Rector\Core\ValueObject\Bootstrap\BootstrapConfigs
    {
        ...
        if (\in_array($argvInput->getFirstArgument(), ['generate', 'g', 'create', 'c'], \true)) {
            ...
            if (\file_exists($rectorRecipeFilePath)) {
                $configFileInfos[] = new \Typo3RectorPrefix20210405\Symplify\SmartFileSystem\SmartFileInfo($rectorRecipeFilePath);
            }
        }
