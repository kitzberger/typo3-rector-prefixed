...
final class RemoveColPosParameterRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, \TYPO3\CMS\Backend\Domain\Repository\Localization\LocalizationRepository::class)) {
            return null;
        }
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Remove parameter colPos from methods.', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
