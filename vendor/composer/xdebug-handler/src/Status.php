...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \Typo3RectorPrefix20210324\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \Typo3RectorPrefix20210324\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \Typo3RectorPrefix20210324\Psr\Log\LogLevel::WARNING);
    }
    ...
    {
        $this->output($this->getLoadedMessage());
        \Typo3RectorPrefix20210324\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    private function reportRestarted()
    {
        $loaded = $this->getLoadedMessage();
        $text = \sprintf('Restarted (%d ms). %s', $this->time, $loaded);
        $level = $this->loaded ? \Typo3RectorPrefix20210324\Psr\Log\LogLevel::WARNING : null;
