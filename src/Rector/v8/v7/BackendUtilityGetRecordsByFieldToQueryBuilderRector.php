...
PHP
    ...
    private function addQueryBuilderNode(\PhpParser\Node\Expr\StaticCall $node, \PhpParser\Node $positionNode) : void
    {
        ...
        if (null === $queryBuilderArgument || 'null' === $this->valueResolver->getValue($queryBuilderArgument->value)) {
            ...
            $queryBuilder = $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, self::MAKE_INSTANCE, [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Core\Database\ConnectionPool::class)]), 'getQueryBuilderForTable', [$table]);
        } else {
    ...
    private function addQueryWhereNode(string $queryBuilderVariableName, \PhpParser\Node\Expr\StaticCall $node, \PhpParser\Node $positionNode) : void
    {
        ...
        $whereClauseNode = $this->nodeFactory->createMethodCall($queryBuilderVariableName, 'andWhere', [$this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Database\Query\QueryHelper::class, 'stripLogicalOperatorPrefix', [$node->args[3]])]);
        if ($whereClause) {
    ...
    private function addQueryGroupByNode(string $queryBuilderVariableName, \PhpParser\Node\Expr\StaticCall $node, \PhpParser\Node $positionNode) : void
    {
        ...
        $groupByNode = $this->nodeFactory->createMethodCall($queryBuilderVariableName, 'groupBy', [$this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Database\Query\QueryHelper::class, 'parseGroupBy', [$node->args[4]])]);
        if ($groupBy) {
    ...
    private function addOrderByNode(string $queryBuilderVariableName, \PhpParser\Node\Expr\StaticCall $node, \PhpParser\Node $positionNode) : void
    {
        ...
        $orderByNode = new \PhpParser\Node\Stmt\Foreach_($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Database\Query\QueryHelper::class, 'parseOrderBy', [$orderByArgument->value]), new \PhpParser\Node\Expr\Variable('orderPair'));
    ...
    private function addLimitNode(string $queryBuilderVariableName, \PhpParser\Node\Expr\StaticCall $node, \PhpParser\Node $positionNode) : void
    {
        ...
        $limitNode->stmts[] = new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign(new \PhpParser\Node\Expr\Variable(self::LIMIT_OFFSET_AND_MAX), $this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'intExplode', [new \PhpParser\Node\Scalar\String_(','), new \PhpParser\Node\Expr\Variable('limit')])));
