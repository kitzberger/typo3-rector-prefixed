...
final class PSR4NamespaceMatcher implements \Rector\PSR4\Contract\PSR4AutoloadNamespaceMatcherInterface
{
    ...
    public function getExpectedNamespace(\PhpParser\Node $node) : ?string
    {
        ...
        foreach ($psr4Autoloads as $namespace => $path) {
            ...
            foreach ($paths as $singlePath) {
                ...
                if (!\Typo3RectorPrefix20210331\Nette\Utils\Strings::startsWith($smartFileInfo->getRelativeDirectoryPath(), $singlePath)) {
                    continue;
                }
     ...
     */
    private function resolveExtraNamespace(\Typo3RectorPrefix20210331\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, string $path) : string
    {
        $extraNamespace = \Typo3RectorPrefix20210331\Nette\Utils\Strings::substring($smartFileInfo->getRelativeDirectoryPath(), \Typo3RectorPrefix20210331\Nette\Utils\Strings::length($path) + 1);
        $extraNamespace = \Typo3RectorPrefix20210331\Nette\Utils\Strings::replace($extraNamespace, '#/#', '\\');
