...
final class PageNotFoundAndErrorHandlingRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Page Not Found And Error handling in Frontend', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
...
PHP
    ...
    private function createResponse(\PhpParser\Node\Expr\MethodCall $node) : ?\PhpParser\Node
    {
        ...
        return new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign(new \PhpParser\Node\Expr\Variable('response'), $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Frontend\Controller\ErrorController::class)]), self::MAP_METHODS[$methodCall], $arguments)));
    }
    private function throwException() : \PhpParser\Node
    {
        return new \PhpParser\Node\Stmt\Throw_(new \PhpParser\Node\Expr\New_(new \PhpParser\Node\Name(\TYPO3\CMS\Core\Http\ImmediateResponseException::class), $this->nodeFactory->createArgs([new \PhpParser\Node\Expr\Variable('response')])));
    }
    private function refactorCheckPageUnavailableHandlerMethod() : \PhpParser\Node
    {
        ...
        return new \PhpParser\Node\Expr\BinaryOp\BooleanAnd($pageUnavailableHandling, new \PhpParser\Node\Expr\BooleanNot($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'cmpIP', [$this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'getIndpEnv', [new \PhpParser\Node\Scalar\String_('REMOTE_ADDR')]), $devIpMask])));
    }
    private function refactorPageErrorHandlerIfPossible(\PhpParser\Node\Expr\MethodCall $node) : ?\PhpParser\Node
    {
        ...
        if (null !== $message) {
            return new \PhpParser\Node\Stmt\Echo_([$this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Core\Controller\ErrorPageController::class)]), 'errorAction', ['Page Not Found', $message])]);
        }
        return null;
    }
