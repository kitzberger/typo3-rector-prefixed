...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node->var, 'Typo3RectorPrefix20210324\\Nette\\Http\\Request')) {
            return null;
        }
        if (!$this->isName($node->name, 'getHeader')) {
            return null;
        }
        $requestName = $this->classMethodManipulator->addMethodParameterIfMissing($node, \Typo3RectorPrefix20210324\Symfony\Component\HttpFoundation\Request::class, ['request', 'symfonyRequest']);
