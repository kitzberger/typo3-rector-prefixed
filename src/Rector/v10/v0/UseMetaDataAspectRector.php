...
final class UseMetaDataAspectRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, \TYPO3\CMS\Core\Resource\File::class)) {
            return null;
        }
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Use $fileObject->getMetaData()->get() instead of $fileObject->_getMetaData()', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
$fileObject = new File();
