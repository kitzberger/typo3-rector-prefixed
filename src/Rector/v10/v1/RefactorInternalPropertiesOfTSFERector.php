...
CODE_SAMPLE
    ...
    private function getRelevantParametersFromCacheHashCalculator() : \PhpParser\Node
    {
        ...
        $ifNode->stmts[] = new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign(new \PhpParser\Node\Expr\Variable(self::RELEVANT_PARAMETERS_FOR_CACHING_FROM_PAGE_ARGUMENTS), $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Frontend\Page\CacheHashCalculator::class)]), 'getRelevantParameters', [$this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\HttpUtility::class, 'buildQueryString', [new \PhpParser\Node\Expr\Variable(self::QUERY_PARAMS)])])));
        return $ifNode;
    }
