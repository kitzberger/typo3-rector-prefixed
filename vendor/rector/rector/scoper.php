...
$dateTime = \Typo3RectorPrefix20210413\Nette\Utils\DateTime::from('now');
...
return [\Rector\Compiler\ValueObject\ScoperOption::PREFIX => 'RectorPrefix' . $timestamp, \Rector\Compiler\ValueObject\ScoperOption::WHITELIST => \Rector\Compiler\PhpScoper\StaticEasyPrefixer::getExcludedNamespacesAndClasses(), \Rector\Compiler\ValueObject\ScoperOption::PATCHERS => [
    ...
    function (string $filePath, string $prefix, string $content) use($filePathsToRemoveNamespace) : string {
        ...
        foreach ($filePathsToRemoveNamespace as $filePathToRemoveNamespace) {
            if (\Typo3RectorPrefix20210413\Nette\Utils\Strings::endsWith($filePath, $filePathToRemoveNamespace)) {
                return \Typo3RectorPrefix20210413\Nette\Utils\Strings::replace($content, $prefixedNamespacePattern, '');
            }
        }
        return $content;
    },
    ...
    function (string $filePath, string $prefix, string $content) : string {
        if (!\Typo3RectorPrefix20210413\Nette\Utils\Strings::endsWith($filePath, 'vendor/jean85/pretty-package-versions/src/PrettyVersions.php')) {
            return $content;
        }
        // see https://regex101.com/r/v8zRMm/1
        return \Typo3RectorPrefix20210413\Nette\Utils\Strings::replace($content, '#' . $prefix . '\\\\Composer\\\\InstalledVersions#', 'Typo3RectorPrefix20210413\\Composer\\InstalledVersions');
    },
    // fixes https://github.com/rectorphp/rector/issues/6007
    function (string $filePath, string $prefix, string $content) : string {
        if (!\Typo3RectorPrefix20210413\Nette\Utils\Strings::contains($filePath, 'vendor/')) {
            return $content;
        }
        // @see https://regex101.com/r/lBV8IO/2
        $fqcnReservedPattern = \sprintf('#(\\\\)?%s\\\\(parent|self|static)#m', $prefix);
        $matches = \Typo3RectorPrefix20210413\Nette\Utils\Strings::matchAll($content, $fqcnReservedPattern);
        if (!$matches) {
            return $content;
        }
    ...
    function (string $filePath, string $prefix, string $content) : string {
        ...
        if (!\Typo3RectorPrefix20210413\Nette\Utils\Strings::match($filePath, '#vendor/symfony/polyfill-php\\d{2}/Resources/stubs#')) {
            return $content;
        }
        // @see https://regex101.com/r/x5Ukrx/1
        $namespace = \sprintf('#namespace %s;#m', $prefix);
        return \Typo3RectorPrefix20210413\Nette\Utils\Strings::replace($content, $namespace);
    },
    // unprefix string classes, as they're string on purpose - they have to be checked in original form, not prefixed
    function (string $filePath, string $prefix, string $content) : string {
        // skip vendor, expect rector packages
        if (\Typo3RectorPrefix20210413\Nette\Utils\Strings::contains($filePath, 'vendor/') && !\Typo3RectorPrefix20210413\Nette\Utils\Strings::contains($filePath, 'vendor/rector')) {
            return $content;
        }
        // skip bin/rector.php for composer autoload class
        if (\Typo3RectorPrefix20210413\Nette\Utils\Strings::endsWith($filePath, 'bin/rector.php')) {
            return $content;
        }
        // skip scoper-autoload
        if (\Typo3RectorPrefix20210413\Nette\Utils\Strings::endsWith($filePath, 'vendor/scoper-autoload.php')) {
            return $content;
        }
    ...
    function (string $filePath, string $prefix, string $content) : string {
        // scoper missed PSR-4 autodiscovery in Symfony
        if (!\Typo3RectorPrefix20210413\Nette\Utils\Strings::endsWith($filePath, 'config.php') && !\Typo3RectorPrefix20210413\Nette\Utils\Strings::endsWith($filePath, 'services.php')) {
            return $content;
        }
        // skip "Rector\\" namespace
        if (\Typo3RectorPrefix20210413\Nette\Utils\Strings::contains($content, '$services->load(\'Rector')) {
            return $content;
        }
        return \Typo3RectorPrefix20210413\Nette\Utils\Strings::replace($content, '#services\\->load\\(\'#', 'services->load(\'' . $prefix . '\\');
    },
]];
