...
final class DecoratedConsoleOutputFormatter implements \Rector\ChangesReporting\Contract\Output\OutputFormatterInterface
{
     ...
     */
    private $lexer;
    public function __construct(\Rector\ChangesReporting\Contract\Output\OutputFormatterInterface $consoleOutputFormatter, \Rector\Core\PhpParser\Printer\BetterStandardPrinter $betterStandardPrinter, \Rector\Core\Configuration\Configuration $configuration, \Typo3RectorPrefix20210401\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle, \Rector\BetterPhpDocParser\PhpDocParser\BetterPhpDocParser $betterPhpDocParser, \Typo3RectorPrefix20210401\Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider, \PHPStan\PhpDocParser\Lexer\Lexer $lexer)
    {
    ...
    {
        if (\false === (bool) $this->parameterProvider->provideParameter(\Ssch\TYPO3Rector\Configuration\Typo3Option::OUTPUT_CHANGELOG)) {
        ...
        if ($changeCount > 0) {
            $message .= \sprintf(' %d file%s %s.', $changeCount, $changeCount > 1 ? 's' : '', $this->configuration->isDryRun() ? 'would have changed (dry-run)' : (1 === $changeCount ? 'has' : 'have') . ' been changed');
        }
                ...
                foreach ($fileDiff->getRectorClasses() as $rectorClass) {
                    ...
                    try {
                        ...
                        if (\count($seeTags) > 0) {
                            ...
                            if ($firstSeeTag instanceof \PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocTagNode) {
                                $appliedRules[$appliedRuleKey] = \sprintf('%s (%s)', $rectorClass, (string) $firstSeeTag->value);
                            }
                        }
    ...
    private function reportErrors(array $errors) : void
    {
        foreach ($errors as $error) {
            ...
            $message = \sprintf('Could not process "%s" file%s, due to: %s"%s".', $error->getFileInfo()->getRelativeFilePathFromCwd(), $error->getRectorClass() ? ' by "' . $error->getRectorClass() . '"' : '', \PHP_EOL, $errorMessage);
            if ($error->getLine()) {
    ...
    {
        $errorMessage = \Typo3RectorPrefix20210401\Nette\Utils\Strings::replace($errorMessage, '#' . \preg_quote(\getcwd(), '#') . '/#');
        return \Typo3RectorPrefix20210401\Nette\Utils\Strings::replace($errorMessage, self::ON_LINE_REGEX, ':');
    }
    private function reportRemovedNodes(\Rector\ChangesReporting\Application\ErrorAndDiffCollector $errorAndDiffCollector) : void
    {
        ...
        if ($this->symfonyStyle->isVeryVerbose()) {
            ...
            foreach ($errorAndDiffCollector->getRemovedNodes() as $removedNode) {
                ...
                $printedNode = '-' . \Typo3RectorPrefix20210401\Nette\Utils\Strings::replace($printedNode, '#\\n#', "\n-");
