...
final class ComposerNamespaceMatcher
{
    ...
    public function __construct(\Typo3RectorPrefix20210411\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem, \Typo3RectorPrefix20210411\Symplify\ComposerJsonManipulator\ComposerJsonFactory $composerJsonFactory)
    {
    ...
    public function matchNamespaceForLocation(string $path) : ?string
    {
        ...
        foreach ($autoload['psr-4'] ?? [] as $namespace => $directory) {
            ...
            foreach ($directory as $singleDirectory) {
                if (!\Typo3RectorPrefix20210411\Nette\Utils\Strings::startsWith($path, $singleDirectory)) {
                    continue;
                }
