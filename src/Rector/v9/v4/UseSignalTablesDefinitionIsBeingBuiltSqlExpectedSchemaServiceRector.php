...
final class UseSignalTablesDefinitionIsBeingBuiltSqlExpectedSchemaServiceRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, \TYPO3\CMS\Extbase\SignalSlot\Dispatcher::class)) {
            return null;
        }
        if (!$this->isName($node->name, 'connect')) {
            return null;
        }
        if (!$this->classConstAnalyzer->isClassConstReference($node->args[0]->value, \TYPO3\CMS\Extensionmanager\Utility\InstallUtility::class)) {
            return null;
        }
        if (!$this->valueResolver->isValue($node->args[1]->value, 'tablesDefinitionIsBeingBuilt')) {
            return null;
        }
        $node->args[0]->value = $this->nodeFactory->createClassConstReference(\TYPO3\CMS\Install\Service\SqlExpectedSchemaService::class);
        return $node;
    }
    /**
     * @codeCoverageIgnore
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Use the signal tablesDefinitionIsBeingBuilt of class SqlExpectedSchemaService', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
