...
CODE_SAMPLE
     ...
     */
    private function refactorParamForSelfAndSiblings(\PhpParser\Node\Stmt\ClassMethod $classMethod, int $position, \PHPStan\Analyser\Scope $scope) : void
    {
     ...
     */
    private function refactorParameters(array $parameterTypesByParentClassLikes, string $methodName, int $paramPosition) : void
    {
        foreach (\array_keys($parameterTypesByParentClassLikes) as $className) {
            $classLike = $this->nodeRepository->findClassLike($className);
            if (!$classLike instanceof \PhpParser\Node\Stmt\ClassLike) {
