...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if (\Typo3RectorPrefix20210409\Nette\Utils\Strings::contains($numberNodeValue, '+')) {
            return null;
        }
         ...
         * Then, add it again.
         */
        if ($node instanceof \PhpParser\Node\Scalar\DNumber && !\Typo3RectorPrefix20210409\Nette\Utils\Strings::contains($node->value, '.')) {
            $node->value .= '.0';
        }
        return $node;
    }
