...
final class KeywordHighlighter
{
    ...
    {
        $words = \Typo3RectorPrefix20210318\Nette\Utils\Strings::split($content, '# #');
        foreach ($words as $key => $word) {
            if (!$this->isKeywordToHighlight($word)) {
                continue;
            }
            $words[$key] = \Typo3RectorPrefix20210318\Nette\Utils\Strings::replace('`' . $word . '`', self::COMMA_SPLIT_REGEX, function (array $match) : string {
    ...
    {
        if (\Typo3RectorPrefix20210318\Nette\Utils\Strings::match($word, self::ANNOTATION_REGEX)) {
            return \true;
        }
        // already in code quotes
        if (\Typo3RectorPrefix20210318\Nette\Utils\Strings::startsWith($word, '`')) {
            return \false;
        }
        if (\Typo3RectorPrefix20210318\Nette\Utils\Strings::endsWith($word, '`')) {
            return \false;
        }
        // part of normal text
        ...
        if ((bool) \Typo3RectorPrefix20210318\Nette\Utils\Strings::match($word, self::VARIABLE_CALL_OR_VARIABLE_REGEX)) {
            return \true;
        }
        return (bool) \Typo3RectorPrefix20210318\Nette\Utils\Strings::match($word, self::STATIC_CALL_REGEX);
    }
    private function isFunctionOrClass(string $word) : bool
    {
        if (\Typo3RectorPrefix20210318\Nette\Utils\Strings::match($word, self::METHOD_NAME_REGEX)) {
            return \true;
        }
        ...
        if ($this->classLikeExistenceChecker->doesClassLikeExist($word)) {
            // not a class
            if (!\Typo3RectorPrefix20210318\Nette\Utils\Strings::contains($word, '\\')) {
