...
use Typo3RectorPrefix20210410\Swift_Attachment;
use Typo3RectorPrefix20210410\Swift_Image;
use Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample;
use Symplify\RuleDocGenerator\ValueObject\RuleDefinition;
use TYPO3\CMS\Core\Mail\MailMessage;
/**
 * @see https://docs.typo3.org/c/typo3/cms-core/10.2/en-us/Changelog/10.0/Feature-88643-NewMailAPIBasedOnSymfonymailerAndSymfonymime.html
...
CODE_SAMPLE
    ...
    private function refactorAttachMethod(\PhpParser\Node\Expr\MethodCall $node) : ?\PhpParser\Node
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($firstArgument, new \PHPStan\Type\ObjectType(\Typo3RectorPrefix20210410\Swift_Attachment::class))) {
            return null;
        }
    ...
    private function refactorEmbedMethod(\PhpParser\Node\Expr\MethodCall $node) : ?\PhpParser\Node
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($firstArgument, new \PHPStan\Type\ObjectType(\Typo3RectorPrefix20210410\Swift_Image::class))) {
            return null;
        }
        if (!$this->isName($firstArgument->name, 'fromPath')) {
            return null;
