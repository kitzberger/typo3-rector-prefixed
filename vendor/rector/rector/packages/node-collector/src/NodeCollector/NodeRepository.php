...
final class NodeRepository
{
    ...
    {
        if (\Typo3RectorPrefix20210326\Nette\Utils\Strings::contains($methodName, '\\')) {
    ...
    {
        $calls = \Typo3RectorPrefix20210326\Nette\Utils\Arrays::flatten($this->callsByTypeAndMethod);
        return \array_filter($calls, function (\PhpParser\Node $node) : bool {
    ...
    public function findClassesBySuffix(string $suffix) : array
    {
        ...
        foreach ($this->parsedNodeCollector->getClasses() as $className => $classNode) {
            if (!\Typo3RectorPrefix20210326\Nette\Utils\Strings::endsWith($className, $suffix)) {
                continue;
            }
