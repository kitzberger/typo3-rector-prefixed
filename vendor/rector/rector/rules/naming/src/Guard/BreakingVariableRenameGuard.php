...
use Typo3RectorPrefix20210324\Ramsey\Uuid\UuidInterface;
...
final class BreakingVariableRenameGuard
{
    ...
    public function shouldSkipVariable(string $currentName, string $expectedName, \PhpParser\Node\FunctionLike $functionLike, \PhpParser\Node\Expr\Variable $variable) : bool
    {
        ...
        if (\Typo3RectorPrefix20210324\Nette\Utils\Strings::endsWith($currentName, \ucfirst($expectedName))) {
            return \true;
        }
    ...
    {
        return $this->nodeTypeResolver->isObjectType($param, \Typo3RectorPrefix20210324\Ramsey\Uuid\UuidInterface::class);
    }
    /**
     * @TODO Remove once ParamRenamer created
     */
    private function isDateTimeAtNamingConvention(\PhpParser\Node\Param $param) : bool
    {
        ...
        return (bool) \Typo3RectorPrefix20210324\Nette\Utils\Strings::match($currentName, self::AT_NAMING_REGEX . '');
    }
}
