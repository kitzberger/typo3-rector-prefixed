            ...
            foreach ($fieldValue->value->items as $configValue) {
                ...
                foreach ($configValue->value->items as $configItemValue) {
                    ...
                    foreach ($configItemValue->value->items as $wizardItemValue) {
                        ...
                        if ($validWizard || \Typo3RectorPrefix20210228\Nette\Utils\Strings::startsWith($this->valueResolver->getValue($wizardItemValue->key), '_') || $isRte) {
                            --$remainingWizards;
                        }
                        if (!$validWizard) {
                            continue;
                        ...
                        if (\array_key_exists($wizardItemValueKey, self::MAP_WIZARD_TO_RENDER_TYPE)) {
                            $configValue->value->items[] = new \PhpParser\Node\Expr\ArrayItem(new \PhpParser\Node\Scalar\String_(self::MAP_WIZARD_TO_RENDER_TYPE[$wizardItemValueKey]), new \PhpParser\Node\Scalar\String_('renderType'));
                        }
                        foreach ($wizardItemValue->value->items as $wizardItemSubValue) {
                            ...
                            if ('angle' === $wizardItemValueKey && !$this->valueResolver->isValue($wizardItemSubValue->key, 'type')) {
                                ...
                                $configValue->value->items[] = new \PhpParser\Node\Expr\ArrayItem(new \PhpParser\Node\Expr\Array_([new \PhpParser\Node\Expr\ArrayItem($wizardItemSubValue->value, $wizardItemSubValue->key)]), new \PhpParser\Node\Scalar\String_(self::MAP_WIZARD_TO_CUSTOM_TYPE[$wizardItemValueKey]));
                            } elseif ('suggest' === $wizardItemValueKey && !$this->valueResolver->isValue($wizardItemSubValue->key, 'type')) {
                                $configValue->value->items[] = new \PhpParser\Node\Expr\ArrayItem(new \PhpParser\Node\Expr\Array_([new \PhpParser\Node\Expr\ArrayItem($wizardItemSubValue->value, $wizardItemSubValue->key)]), new \PhpParser\Node\Scalar\String_(self::MAP_WIZARD_TO_CUSTOM_TYPE[$wizardItemValueKey]));
                            }
                        ...
                        if ([] !== $customTypeOptions && \array_key_exists($wizardItemValueKey, self::MAP_WIZARD_TO_CUSTOM_TYPE)) {
                            $configValue->value->items[] = new \PhpParser\Node\Expr\ArrayItem($this->nodeFactory->createArray($customTypeOptions), new \PhpParser\Node\Scalar\String_(self::MAP_WIZARD_TO_CUSTOM_TYPE[$wizardItemValueKey]));
                        }
                    }
                    if ([] !== $fieldControl) {
                        $configValue->value->items[] = new \PhpParser\Node\Expr\ArrayItem($this->nodeFactory->createArray($fieldControl), new \PhpParser\Node\Scalar\String_('fieldControl'));
                    }
