...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        // skip phar
        if (\Typo3RectorPrefix20210418\Nette\Utils\Strings::startsWith($includeValue, 'phar://')) {
            return null;
        }
        // skip absolute paths
        if (\Typo3RectorPrefix20210418\Nette\Utils\Strings::startsWith($includeValue, '/')) {
            return null;
        }
        // add preslash to string
        if (\Typo3RectorPrefix20210418\Nette\Utils\Strings::startsWith($includeValue, './')) {
            $node->expr->value = \Typo3RectorPrefix20210418\Nette\Utils\Strings::substring($includeValue, 1);
        } else {
