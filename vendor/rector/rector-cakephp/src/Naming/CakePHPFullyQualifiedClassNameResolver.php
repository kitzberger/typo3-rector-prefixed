...
final class CakePHPFullyQualifiedClassNameResolver
{
    ...
    public function resolveFromPseudoNamespaceAndShortClassName(string $pseudoNamespace, string $shortClass) : string
    {
        ...
        if (\Typo3RectorPrefix20210409\Nette\Utils\Strings::match($pseudoNamespace, self::LIB_NAMESPACE_PART_REGEX)) {
            $pseudoNamespace = \Typo3RectorPrefix20210409\Nette\Utils\Strings::replace($pseudoNamespace, '#\\\\Lib#', '');
        }
        ...
        if (\Typo3RectorPrefix20210409\Nette\Utils\Strings::contains($pseudoNamespace, '\\') && !\Typo3RectorPrefix20210409\Nette\Utils\Strings::match($pseudoNamespace, self::PLUGIN_OR_LIB_REGEX)) {
    ...
    {
        return \Typo3RectorPrefix20210409\Nette\Utils\Strings::replace($pseudoNamespace, self::SLASH_REGEX, '\\');
    }
}
