...
abstract class AbstractPhpDocNodeFactory
{
    ...
    {
        $match = \Typo3RectorPrefix20210227\Nette\Utils\Strings::match($annotationContent, self::OPENING_SPACE_REGEX);
        $openingSpace = $match['opening_space'] ?? '';
        $match = \Typo3RectorPrefix20210227\Nette\Utils\Strings::match($annotationContent, self::CLOSING_SPACE_REGEX);
    ...
    {
        return \Typo3RectorPrefix20210227\Nette\Utils\Strings::replace($targetEntity, self::CLASS_CONST_REGEX, '');
    }
}
