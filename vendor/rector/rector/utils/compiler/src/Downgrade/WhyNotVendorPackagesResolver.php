...
final class WhyNotVendorPackagesResolver
{
    ...
    private function resolveWhyNotPackageNames(string $targetPhpVersion) : array
    {
        ...
        $matches = \Typo3RectorPrefix20210413\Nette\Utils\Strings::matchAll($commandOutput, self::PACKAGE_NAME_REGEX);
        $packageNames = [];
    ...
    private function resolvePackageNamesToPaths() : array
    {
        ...
        $matches = \Typo3RectorPrefix20210413\Nette\Utils\Strings::matchAll($commandOutput, self::PACKAGE_NAME_TO_PACKAGE_PATH_REGEX);
        $packageNameToPath = [];
        foreach ($matches as $match) {
    ...
    {
        $process = new \Typo3RectorPrefix20210413\Symfony\Component\Process\Process($commandLine);
        $process->run();
