...
final class ComposerNamespaceMatcher
{
    ...
    public function __construct(\Typo3RectorPrefix20210402\Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem, \Typo3RectorPrefix20210402\Symplify\ComposerJsonManipulator\ComposerJsonFactory $composerJsonFactory)
    {
    ...
    public function matchNamespaceForLocation(string $path) : ?string
    {
        ...
        foreach ($autoload['psr-4'] ?? [] as $namespace => $directory) {
            if (!\Typo3RectorPrefix20210402\Nette\Utils\Strings::startsWith($path, $directory)) {
                continue;
            }
