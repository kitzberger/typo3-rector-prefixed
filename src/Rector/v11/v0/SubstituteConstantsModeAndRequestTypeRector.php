...
final class SubstituteConstantsModeAndRequestTypeRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Substitute TYPO3_MODE and TYPO3_REQUESTTYPE constants', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
...
PHP
    ...
    {
        return $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Http\ApplicationType::class, 'fromRequest', $arguments), 'isBackend');
    }
    private function createIsFrontendCall(array $arguments) : \PhpParser\Node\Expr\MethodCall
    {
        return $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Http\ApplicationType::class, 'fromRequest', $arguments), 'isFrontend');
    }
}
