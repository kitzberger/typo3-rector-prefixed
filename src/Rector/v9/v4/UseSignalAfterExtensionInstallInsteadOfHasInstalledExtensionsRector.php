...
final class UseSignalAfterExtensionInstallInsteadOfHasInstalledExtensionsRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($node, new \PHPStan\Type\ObjectType(\TYPO3\CMS\Extbase\SignalSlot\Dispatcher::class))) {
            return null;
        }
        if (!$this->isName($node->name, 'connect')) {
            return null;
        }
        if (!$this->classConstAnalyzer->isClassConstReference($node->args[0]->value, \TYPO3\CMS\Extensionmanager\Service\ExtensionManagementService::class)) {
            return null;
        }
        if (!$this->valueResolver->isValue($node->args[1]->value, 'hasInstalledExtensions')) {
            return null;
        }
        $node->args[0]->value = $this->nodeFactory->createClassConstReference(\TYPO3\CMS\Extensionmanager\Utility\InstallUtility::class);
