...
final class ConstantNodeFactory
{
    ...
    public function __construct(\Typo3RectorPrefix20210308\Symplify\PhpConfigPrinter\Contract\YamlFileContentProviderInterface $yamlFileContentProvider)
    {
    ...
    public function createConstantIfValue(string $value) : ?\PhpParser\Node\Expr
    {
        ...
        foreach (\array_keys($definedConstants) as $constantName) {
            ...
            if (!\Typo3RectorPrefix20210308\Nette\Utils\Strings::match($yamlContent, $constantDefinitionPattern)) {
                continue;
            }
