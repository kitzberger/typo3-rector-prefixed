...
namespace Typo3RectorPrefix20210415\Symplify\SmartFileSystem;

use Typo3RectorPrefix20210415\Nette\Utils\Strings;
use Typo3RectorPrefix20210415\Symfony\Component\Finder\SplFileInfo;
use Typo3RectorPrefix20210415\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use Typo3RectorPrefix20210415\Symplify\EasyTesting\StaticFixtureSplitter;
use Typo3RectorPrefix20210415\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
 ...
 */
final class SmartFileInfo extends \Typo3RectorPrefix20210415\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    {
        $this->smartFileSystem = new \Typo3RectorPrefix20210415\Symplify\SmartFileSystem\SmartFileSystem();
        // accepts also dirs
        if (!\file_exists($filePath)) {
            throw new \Typo3RectorPrefix20210415\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
        // real path doesn't work in PHAR: https://www.php.net/manual/en/function.realpath.php
        if (\Typo3RectorPrefix20210415\Nette\Utils\Strings::startsWith($filePath, 'phar://')) {
    ...
    {
        return \Typo3RectorPrefix20210415\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory(string $directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \Typo3RectorPrefix20210415\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\Typo3RectorPrefix20210415\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\Typo3RectorPrefix20210415\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
    ...
    {
        return \Typo3RectorPrefix20210415\Nette\Utils\Strings::endsWith($this->getNormalizedRealPath(), $string);
    }
    ...
    {
        return \Typo3RectorPrefix20210415\Nette\Utils\Strings::startsWith($this->getNormalizedRealPath(), $partialPath);
    }
