...
class StaticStringy
{
    ...
    public static function __callStatic($name, $arguments)
    {
        if (!static::$methodArgs) {
            $stringyClass = new \ReflectionClass('Typo3RectorPrefix20210411\\Stringy\\Stringy');
        ...
        $stringy = \Typo3RectorPrefix20210411\Stringy\Stringy::create($str, $encoding);
        $result = \call_user_func_array([$stringy, $name], $args);
        $cast = function ($val) {
            if (\is_object($val) && $val instanceof \Typo3RectorPrefix20210411\Stringy\Stringy) {
                return (string) $val;
            } else {
                return $val;
            }
