...
CODE_SAMPLE
    ...
    {
        return new \PhpParser\Node\Expr\BinaryOp\Concat(new \PhpParser\Node\Expr\BinaryOp\Concat($this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Backend\Routing\UriBuilder::class)]), 'buildUriFromRoute', [$this->nodeFactory->createArg('record_edit')]), $firstArgument->value), new \PhpParser\Node\Scalar\String_('&returnUrl='));
    }
    private function createRequestUriCall() : \PhpParser\Node\Expr\FuncCall
    {
        return new \PhpParser\Node\Expr\FuncCall(new \PhpParser\Node\Name('rawurlencode'), [$this->nodeFactory->createArg($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'getIndpEnv', [$this->nodeFactory->createArg('REQUEST_URI')]))]);
    }
}
