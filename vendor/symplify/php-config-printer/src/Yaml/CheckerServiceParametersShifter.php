...
namespace Typo3RectorPrefix20210329\Symplify\PhpConfigPrinter\Yaml;
...
final class CheckerServiceParametersShifter
{
    ...
    private function processServices(array $services) : array
    {
        foreach ($services as $serviceName => $serviceDefinition) {
            ...
            if (\Typo3RectorPrefix20210329\Nette\Utils\Strings::endsWith($serviceName, 'Fixer')) {
                $services = $this->processFixer($services, $serviceName, $serviceDefinition);
            }
            if (\Typo3RectorPrefix20210329\Nette\Utils\Strings::endsWith($serviceName, 'Sniff')) {
    ...
    {
        if (\Typo3RectorPrefix20210329\Nette\Utils\Strings::endsWith($checker, 'Fixer')) {
            return \true;
        }
        return \Typo3RectorPrefix20210329\Nette\Utils\Strings::endsWith($checker, 'Sniff');
    }
    /**
     * @param mixed[] $services
    ...
    private function escapeValue($value)
    {
        ...
        return \Typo3RectorPrefix20210329\Nette\Utils\Strings::replace($value, '#^@#', '@@');
    }
}
