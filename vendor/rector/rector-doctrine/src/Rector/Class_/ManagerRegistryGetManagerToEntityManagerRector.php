...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        $registryCalledMethods = $this->methodCallNameOnTypeResolver->resolve($node, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210413\\Doctrine\\Common\\Persistence\\ManagerRegistry'));
        ...
        $this->addConstructorDependencyWithProperty($node, $constructorClassMethod, self::ENTITY_MANAGER, new \Rector\StaticTypeMapper\ValueObject\Type\FullyQualifiedObjectType('Typo3RectorPrefix20210413\\Doctrine\\ORM\\EntityManagerInterface'));
        return $node;
    }
    private function resolveManagerRegistryParam(\PhpParser\Node\Stmt\ClassMethod $classMethod) : ?\PhpParser\Node\Param
    {
        foreach ($classMethod->params as $param) {
            ...
            if (!$this->isName($param->type, 'Typo3RectorPrefix20210413\\Doctrine\\Common\\Persistence\\ManagerRegistry')) {
                continue;
            }
            return $param;
        }
    ...
    private function removeManagerRegistryDependency(\PhpParser\Node\Stmt\Class_ $class, \PhpParser\Node\Stmt\ClassMethod $classMethod, \PhpParser\Node\Param $registryParam) : void
    {
        ...
        $this->dependencyRemover->removeByType($class, $classMethod, $registryParam, 'Typo3RectorPrefix20210413\\Doctrine\\Common\\Persistence\\ManagerRegistry');
    }
    /**
    ...
    private function replaceEntityRegistryVariableWithEntityManagerProperty(\PhpParser\Node\Stmt\Class_ $class) : void
    {
        $this->traverseNodesWithCallable($class->stmts, function (\PhpParser\Node $class) : ?PropertyFetch {
            ...
            if (!$this->isObjectType($class, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210413\\Doctrine\\Common\\Persistence\\ObjectManager'))) {
                return null;
            }
    ...
    private function isRegistryGetManagerMethodCall(\PhpParser\Node\Expr\Assign $assign) : bool
    {
        ...
        if (!$this->isObjectType($assign->expr->var, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210413\\Doctrine\\Common\\Persistence\\ManagerRegistry'))) {
            return \false;
        }
