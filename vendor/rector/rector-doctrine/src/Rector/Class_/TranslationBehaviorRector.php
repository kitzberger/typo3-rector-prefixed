...
CODE_SAMPLE
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        $translatableObjectType = new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210418\\Gedmo\\Translatable\\Translatable');
        if (!$translatableObjectType->isSuperTypeOf($classType)->yes()) {
            return null;
        }
        $this->classManipulator->removeInterface($node, 'Typo3RectorPrefix20210418\\Gedmo\\Translatable\\Translatable');
        $this->classInsertManipulator->addAsFirstTrait($node, 'Typo3RectorPrefix20210418\\Knp\\DoctrineBehaviors\\Model\\Translatable\\TranslatableTrait');
        $node->implements[] = new \PhpParser\Node\Name\FullyQualified('Typo3RectorPrefix20210418\\Knp\\DoctrineBehaviors\\Contract\\Entity\\TranslatableInterface');
    ...
    private function collectAndRemoveTranslatableProperties(\PhpParser\Node\Stmt\Class_ $class) : array
    {
        ...
        foreach ($class->getProperties() as $property) {
            ...
            if ($phpDocInfo->hasByAnnotationClass('Typo3RectorPrefix20210418\\Gedmo\\Mapping\\Annotation\\Locale')) {
                $this->removeNode($property);
                continue;
            }
            $doctrineAnnotationTagValueNode = $phpDocInfo->getByAnnotationClass('Typo3RectorPrefix20210418\\Gedmo\\Mapping\\Annotation\\Translatable');
