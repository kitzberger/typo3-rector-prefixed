...
final class ConfiguredServiceCaseConverter implements \Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\Contract\CaseConverterInterface
{
    /**
     * @var ArgsNodeFactory
     */
    ...
    public function convertToMethodCall($key, $values) : \PhpParser\Node\Stmt\Expression
    {
        ...
        if (isset($values[\Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY])) {
            $valuesForArgs[] = $values[\Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\YamlKey::CLASS_KEY];
        }
        $args = $this->argsNodeFactory->createFromValues($valuesForArgs);
        $methodCall = new \PhpParser\Node\Expr\MethodCall(new \PhpParser\Node\Expr\Variable(\Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\VariableName::SERVICES), \Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\MethodName::SET, $args);
    ...
    public function match(string $rootKey, $key, $values) : bool
    {
        ...
        if ($key === \Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_DEFAULTS) {
            return \false;
        }
        if ($key === \Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\YamlKey::_INSTANCEOF) {
            return \false;
        }
        if (isset($values[\Typo3RectorPrefix20210330\Symplify\PhpConfigPrinter\ValueObject\YamlKey::RESOURCE])) {
            return \false;
        }
    ...
    private function isAlias($values) : bool
    {
        ...
        return \Typo3RectorPrefix20210330\Nette\Utils\Strings::startsWith($values, '@');
    }
}
