...
CODE_SAMPLE
    ...
    {
        if (!$this->isObjectType($node, new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210414\\Doctrine\\ORM\\EntityRepository'))) {
            return null;
        }
        // remove parent class
        $node->extends = null;
        $entityObjectType = $this->entityObjectTypeResolver->resolveFromRepositoryClass($node);
        $genericObjectType = new \PHPStan\Type\Generic\GenericObjectType('Typo3RectorPrefix20210414\\Doctrine\\ORM\\EntityRepository', [$entityObjectType]);
        ...
        $this->classDependencyManipulator->addConstructorDependencyWithCustomAssign($node, 'entityManager', new \PHPStan\Type\ObjectType('Typo3RectorPrefix20210414\\Doctrine\\ORM\\EntityManagerInterface'), $repositoryAssign);
        return $node;
    }
}
