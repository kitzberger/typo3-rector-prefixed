...
final class RefactorRemovedMethodsFromContentObjectRendererRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Refactor removed methods from ContentObjectRenderer.', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
...
PHP
    ...
    private function shouldSkip(\PhpParser\Node\Expr\MethodCall $node) : bool
    {
        ...
        if ($staticType instanceof \PHPStan\Type\TypeWithClassName && \TYPO3\CMS\Frontend\ContentObject\ContentObjectRenderer::class === $staticType->getClassName()) {
            return \false;
        }
