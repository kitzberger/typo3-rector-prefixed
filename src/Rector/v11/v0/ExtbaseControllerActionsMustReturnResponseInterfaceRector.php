...
use Typo3RectorPrefix20210331\Psr\Http\Message\ResponseInterface;
...
final class ExtbaseControllerActionsMustReturnResponseInterfaceRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        $node->returnType = new \PhpParser\Node\Name\FullyQualified(\Typo3RectorPrefix20210331\Psr\Http\Message\ResponseInterface::class);
...
CODE_SAMPLE
    ...
    private function shouldSkip(\PhpParser\Node\Stmt\ClassMethod $node) : bool
    {
        ...
        if (!\Typo3RectorPrefix20210331\Nette\Utils\Strings::endsWith($methodName, 'Action')) {
            return \true;
        }
        if (\Typo3RectorPrefix20210331\Nette\Utils\Strings::startsWith($methodName, 'initialize')) {
            return \true;
        }
    ...
    private function alreadyResponseReturnType(\PhpParser\Node\Stmt\ClassMethod $node) : bool
    {
        foreach ($this->extractReturnCalls($node) as $returnCall) {
            ...
            if (\is_a($returnType->getClassName(), \Typo3RectorPrefix20210331\Psr\Http\Message\ResponseInterface::class, \true)) {
                return \true;
            }
        }
        return \false;
