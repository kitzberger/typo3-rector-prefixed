...
final class ClassAnnotationMatcher
{
    ...
    private function isUseMatchingName(string $tag, \PhpParser\Node\Stmt\UseUse $useUse) : bool
    {
        ...
        return (bool) \Typo3RectorPrefix20210412\Nette\Utils\Strings::match($tag, '#' . $shortNamePattern . '(\\\\[\\w]+)?#i');
    }
    private function resolveName(string $tag, \PhpParser\Node\Stmt\UseUse $useUse) : string
    {
        ...
        $unaliasedShortClass = \Typo3RectorPrefix20210412\Nette\Utils\Strings::substring($tag, \Typo3RectorPrefix20210412\Nette\Utils\Strings::length($useUse->alias->toString()));
        if (\Typo3RectorPrefix20210412\Nette\Utils\Strings::startsWith($unaliasedShortClass, '\\')) {
