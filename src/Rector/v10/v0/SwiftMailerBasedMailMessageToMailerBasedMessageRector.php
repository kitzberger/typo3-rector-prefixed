...
CODE_SAMPLE
    ...
    private function refactorAttachMethod(\PhpParser\Node\Expr\MethodCall $node) : ?\PhpParser\Node
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($firstArgument, \Typo3RectorPrefix20210331\Swift_Attachment::class)) {
            return null;
        }
    ...
    private function refactorEmbedMethod(\PhpParser\Node\Expr\MethodCall $node) : ?\PhpParser\Node
    {
        ...
        if (!$this->nodeTypeResolver->isMethodStaticCallOrClassMethodObjectType($firstArgument, \Typo3RectorPrefix20210331\Swift_Image::class)) {
            return null;
        }
        if (!$this->isName($firstArgument->name, 'fromPath')) {
            return null;
