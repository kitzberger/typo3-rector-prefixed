...
final class RefactorRemovedMarkerMethodsFromContentObjectRendererRector extends \Rector\Core\Rector\AbstractRector
{
    ...
    public function refactor(\PhpParser\Node $node) : ?\PhpParser\Node
    {
        ...
        if ($this->isNames($node->name, ['getSubpart', 'substituteSubpart', 'substituteSubpartArray', 'substituteMarker', 'substituteMarkerArrayCached', 'substituteMarkerArray', 'substituteMarkerInObject', 'substituteMarkerAndSubpartArrayRecursive'])) {
            ...
            $classConstant = $this->nodeFactory->createClassConstReference(\TYPO3\CMS\Core\Service\MarkerBasedTemplateService::class);
        ...
        if ($this->isName($node->name, self::FILL_IN_MARKER_ARRAY)) {
            ...
            return $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Core\Service\MarkerBasedTemplateService::class)]), self::FILL_IN_MARKER_ARRAY, $node->args);
        }
        return null;
    }
    /**
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('Refactor removed Marker-related methods from ContentObjectRenderer.', [new \Symplify\RuleDocGenerator\ValueObject\CodeSample\CodeSample(<<<'PHP'
// build template
