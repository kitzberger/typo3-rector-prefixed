...
CODE_SAMPLE
    ...
    {
        return new \PhpParser\Node\Stmt\Expression(new \PhpParser\Node\Expr\Assign($parentNode->var, $this->nodeFactory->createMethodCall($this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Frontend\Resource\FilePathSanitizer::class)]), 'sanitize', [$filePath])));
    }
    ...
    {
        $makeInstanceOfTimeTracker = $this->nodeFactory->createStaticCall(\TYPO3\CMS\Core\Utility\GeneralUtility::class, 'makeInstance', [$this->nodeFactory->createClassConstReference(\TYPO3\CMS\Core\TimeTracker\TimeTracker::class)]);
    ...
    {
        return new \PhpParser\Node\Stmt\Catch_([new \PhpParser\Node\Name(\TYPO3\CMS\Core\Resource\Exception\InvalidPathException::class), new \PhpParser\Node\Name(\TYPO3\CMS\Core\Resource\Exception\FileDoesNotExistException::class), new \PhpParser\Node\Name(\TYPO3\CMS\Core\Resource\Exception\InvalidFileException::class)], new \PhpParser\Node\Expr\Variable('e'), $stmts);
    }
    private function createCatchBlockToIgnore(\PhpParser\Node\Stmt\Expression $assignmentNodeNull) : \PhpParser\Node\Stmt\Catch_
    {
        return new \PhpParser\Node\Stmt\Catch_([new \PhpParser\Node\Name(\TYPO3\CMS\Core\Resource\Exception\InvalidFileNameException::class)], new \PhpParser\Node\Expr\Variable('e'), [$assignmentNodeNull]);
    }
